...
st.title("サンプルアプリ②: ２つの機能を持つWebアプリを表示します")

st.write("### 動作モード1: 文字数カウント")
st.write("入力フォームにテキストを入力すると、「実行」ボタンを押すことで文字数をカウントできます。")
st.write("### 動作モード2: BMI値の計算")
st.write("身長と体重を入力すると、肥満度を表す体型指数のBMI値を算出できます。")
...
if selected_item == "文字数カウント":
    ...
    height = st.text_input(label="身長（cm）を入力をお願いします。")
    weight = st.text_input(label="体重（kg）を入力をお願いします")
